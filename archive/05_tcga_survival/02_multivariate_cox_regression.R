...
# geneID_included_M and geneID_included_F should come from
# 01_load_and_prepare_tcga_data.R (top motifs per sex)
# If not defined separately, fall back to splitting geneID_included by motif sex

if (!exists("geneID_included_M") || !exists("geneID_included_F")) {
  # Derive from df_split which has a 'sex' column (Male / Female)
...
# --- Helper: wide output (one row per gene, all covariates spread) ------------
...
all_long_results <- list()  # store all long results for permutation test
...
for (motif_sex in names(motif_genesets)) {
  ...
  wide_results <- list()   # collect wide tables across datasets for this motif sex
  ...
  for (dataset_name in names(datasets)) {
    ...
    # Suffix for column names e.g. "MaleMotifs_Males"
    suffix <- paste0(substr(motif_sex, 1, 1), substr(dataset_name, 1, 1))
    # MM = MaleMotifs x Males, MF = MaleMotifs x Females, MA = MaleMotifs x All
    # FM = FemaleMotifs x Males, FF = FemaleMotifs x Females, FA = FemaleMotifs x All
    
    # --- Long format (used for forest + KM plots) ----------------------------
    ...
    write.csv(long_res,
              file.path(out_sub, paste0("cox_long_", motif_sex, "_", dataset_name, ".csv")),
              row.names = FALSE)
    ...
    write.csv(wide_res,
              file.path(out_sub, paste0("cox_wide_", motif_sex, "_", dataset_name, ".csv")),
              row.names = FALSE)
  ...
  # --- Merge wide tables across all 3 datasets for this motif sex -------------
  ...
  write.csv(wide_combined,
            file.path(out_sub, paste0("cox_wide_", motif_sex, "_COMBINED.csv")),
            row.names = FALSE)
...
# Save all long results for downstream permutation test
saveRDS(all_long_results, file.path(output_dir, "all_long_cox_results.rds"))
...
cat("\nOutput structure:\n")
cat("  output/MaleMotifs/   — Cox results, forest plots, KM plots for male motif genes\n")
cat("  output/FemaleMotifs/ — Cox results, forest plots, KM plots for female motif genes\n")
cat("  Each contains long CSV, wide CSV per dataset, and combined wide CSV\n")
cat("\nNext step: run 02b_permutation_test.R\n")