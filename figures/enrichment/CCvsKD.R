...
for (comp in comparisons) {
  ...
  write.table(overlap_KD,
              file.path(output_dir, paste0(comp$sex, "_CC20kb_overlap_KDenriched.txt")),
              row.names = FALSE, col.names = FALSE, quote = FALSE)
  
  write.table(overlap_WT,
              file.path(output_dir, paste0(comp$sex, "_CC20kb_overlap_WTenriched.txt")),
              row.names = FALSE, col.names = FALSE, quote = FALSE)
  ...
  run_enrichr_and_save(
    genes      = overlap_KD,
    label      = paste0(comp$sex, "_CC20kb_overlap_KDenriched"),
  ...
  run_enrichr_and_save(
    genes      = overlap_WT,
    label      = paste0(comp$sex, "_CC20kb_overlap_WTenriched"),
    output_dir = output_dir,
    dbs        = dbs
  )
}
