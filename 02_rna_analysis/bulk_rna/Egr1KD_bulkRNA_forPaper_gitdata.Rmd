...
```{r setup_helpers, message=FALSE, warning=FALSE}
## Libraries + custom helper functions (the "0-config"/utils you used to source).
## Helper scripts are committed to git at bulk_rna/R/.
library(RColorBrewer); library(ggsci); library(ggplot2); library(plotly)
library(DT); library(htmltools); library(biomaRt)
library(limma); library(edgeR); library(Glimma); library(pheatmap)
library(genefilter); library(dplyr)
max_FDR <- 0.05; min_LFC <- 1
rdir <- "C:/Users/loril/Documents/GitHub/Egr1-Llaci/02_rna_analysis/bulk_rna/R"
source(file.path(rdir, "01-build_gene_annotation.R"))
source(file.path(rdir, "02-qc_sequence_effort.R"))
source(file.path(rdir, "03-filter_lowly_expressed.R"))
source(file.path(rdir, "04-PCA_functions.R"))
source(file.path(rdir, "05-volcano_plot.R"))
source(file.path(rdir, "06-Glimma_functions.R"))
source(file.path(rdir, "07-TF_Ligand_Receptor_functions.R"))
```

```{r parameters}
# Save for another time example of how to set working directory with relative path in RMD
# original (author's machine): "C:/Users/loril/Documents/Egr1/Egr1_bulk_RNAseq/Egr1_KD_030525/for paper/"
myWorkingDirectory = "C:/Users/loril/Documents/GitHub/Egr1-Llaci/02_rna_analysis/bulk_rna/output_DE_Egr1KD_gRNA3_vs_Neg1_2x"
...
## raw data read from the copy committed to git (bulk_rna/data_raw/)
...
```{r graphics}
library(ggsci)   # provides pal_nejm()
...
```
Remove outliers, No Treatment Neg2
...
anno_colors <- list(
  ...
  Group = c(
    ...
    NoTreatment_gRNA2_Female = "#C71585",
    NoTreatment_gRNA2_Male = "#00CED1",
