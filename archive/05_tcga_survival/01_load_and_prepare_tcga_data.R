...
motif_female <- read.csv(file.path(motif_directory, "F_all_motifs.csv")) %>%
  arrange(p.adjust) %>%
  slice(1:30)

motif_male <- read.csv(file.path(motif_directory, "M_all_motifs.csv")) %>%
  arrange(p.adjust) %>%
  slice(1:30)
...
# --- Filter genes present in expression ---------------------------------------

geneID_included <- geneID[geneID %in% colnames(tcga_exp)]

missing_genes <- setdiff(geneID, geneID_included)
if (length(missing_genes) > 0) {
  message("Not found in TCGA expression data: ", paste(missing_genes, collapse = ", "))
}

cat("Genes found in TCGA:", length(geneID_included), "/", length(geneID), "\n")

# Summary table
if (length(geneID_included) > 0) {
  apply(as.matrix(tcga_exp[, geneID_included]), 2, summary) %>%
    kbl(caption = "Gene expression summary in all GBM") %>%
    print()
}
...
# --- Add gene expression to phenotype -----------------------------------------

tcga_pheno <- data.frame(tcga_pheno, genes)

# --- Filter: IDH WT + known sex -----------------------------------------------
...
cat("Samples after IDH filtering:", nrow(tcga_pheno), "\n")
print(with(tcga_pheno, table(Gender)))
...
# --- Binary expression (median split) -----------------------------------------

for (gene in genes) {
  tcga_pheno <- tcga_pheno %>%
    mutate(!!paste0(gene, "_binary") :=
             factor(ifelse(!!sym(gene) >= median(!!sym(gene), na.rm = TRUE),
                           "High", "Low"),
                    levels = c("Low", "High")))
}

# Male
tcga_pheno_male2 <- tcga_pheno_male
for (gene in genes) {
  tcga_pheno_male2 <- tcga_pheno_male2 %>%
    mutate(!!paste0(gene, "_binary") :=
             factor(ifelse(!!sym(gene) >= median(!!sym(gene), na.rm = TRUE),
                           "High", "Low"),
                    levels = c("Low", "High")))
...
# Female
tcga_pheno_female2 <- tcga_pheno_female
for (gene in genes) {
  tcga_pheno_female2 <- tcga_pheno_female2 %>%
    mutate(!!paste0(gene, "_binary") :=
             factor(ifelse(!!sym(gene) >= median(!!sym(gene), na.rm = TRUE),
                           "High", "Low"),
                    levels = c("Low", "High")))
...
tcga_pheno_male2  <- factorize_cols(tcga_pheno_male2)
tcga_pheno_female2<- factorize_cols(tcga_pheno_female2)

message("Data preparation complete. Objects ready for downstream scripts.")
...
saveRDS(tcga_pheno, file.path(output_dir, "tcga_pheno_maleANDfemale_allgenes.rds"))
saveRDS(tcga_pheno_male, file.path(output_dir, "tcga_pheno_male_allgenes.rds"))
saveRDS(tcga_pheno_female, file.path(output_dir, "tcga_pheno_female_allgenes.rds"))
